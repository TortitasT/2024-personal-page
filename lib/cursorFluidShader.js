...
let hasSplatted = false

function randomBetween(min, max) {
  return Math.random() * (max - min + 1.0) + min
}

function letWeekChoose(min, max) {
  const dayOfWeek = new Date().getDay()
  const percentage = dayOfWeek / 7
...
let config = {
  ...
  CURL: letWeekChoose(5, Math.PI * 10),
  SPLAT_RADIUS: 0.002,
}
...
function startingSplat() {
  const color = [Math.random() * 10, Math.random() * 10, Math.random() * 10]
  const x = -30
  const y = canvas.height / 1.35
  splat(x, y, 5000, 200, color)
